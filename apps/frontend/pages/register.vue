    ...
    <NuxtLayout name="dashboard">
        <div class="h-full p-6">
            <div class="flex w-full gap-2 md:gap-4 overflow-hidden">
                ...
                <div class="flex flex-1 flex-col overflow-hidden">
                    <div class="flex items-center text-xs gap-0.5 p-1.5 md:text-sm md:p-2">
                        <span class="text-gray-500">敘述</span>
                        <span class="text-red-500">*</span>
                    </div>
            ...
            <div class="flex pt-4 md:pt-6">
                <Button label="送出" class="w-full text-sm md:text-base" @click="submit"/>
            </div>
...
<script setup lang="ts">
    ...
    const { waitSocketConnected } = appStore;
    const { lobbySocket } = storeToRefs(appStore);
    const registerStore = useRegisterStore();
    const { submit } = registerStore;
    const { state } = storeToRefs(registerStore);
    const toast = useToast();

    onMounted(async () => {
        await waitSocketConnected();

        lobbySocket.value?.on('game-registered', (event) => {
            console.log(event);
